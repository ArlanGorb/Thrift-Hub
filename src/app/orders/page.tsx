        ...
        <div className="flex gap-2 mb-6 border-b border-stone-200">
          ...
          >
            ...
            {activeOrders.length > 0 && (
              <span className="ml-1 px-1.5 py-0.5 text-xs bg-blue-100 text-blue-900 rounded-full">{activeOrders.length}</span>
            )}
          </button>
          <button
          ...
          <div className="text-center py-20">
            {activeTab === 'archived' ? (
              ...
              <>
                ...
                <Link
              href="/products"
              className="inline-flex items-center gap-2 bg-blue-900 text-white font-semibold px-6 py-2.5 rounded-xl hover:bg-blue-950 transition-colors"
            >
              Belanja Sekarang
            </Link>
              </>
                ...
                <div className="p-5">
                  <div className="flex flex-wrap items-center justify-between gap-3">
                    ...
                    {order.status === 'completed' && (
                      archivedIds.has(order.id) ? (
                        ...
                        <button
                          ...
                          className="flex items-center gap-1 text-xs text-stone-500 hover:text-blue-900 font-medium transition-colors border border-stone-200 rounded-lg px-3 py-1.5"
                        >
                    ...
                    <button
                      ...
                      className="flex items-center gap-1 text-sm text-blue-900 hover:text-blue-950 font-medium transition-colors"
                    >
                      {expanded === order.id ? (
                            ...
                            {order.status === 'completed' && (
                              <div className="ml-15 pl-15">
                                {reviewedKeys.has(reviewKey(order.id, item)) ? (
                                  ...
                                  <form onSubmit={handleSubmitReview} className="mt-3 p-4 bg-blue-50 border border-blue-300 rounded-xl">
                                    <p className="text-sm font-semibold text-stone-800 mb-3">Ulasan untuk: <span className="text-blue-900">{item.product_name}</span></p>
                                    <div className="mb-3">
                                        ...
                                        {[1,2,3,4,5].map((star) => (
                                          ...
                                          >
                                            <Star className={`w-6 h-6 transition-colors ${
                                              star <= reviewRating ? 'fill-blue-500 text-blue-500' : 'text-stone-300'
                                            }`} />
                                          </button>
                                        ))}
                                        {reviewRating > 0 && (
                                    ...
                                    <div className="mb-3">
                                      ...
                                      <textarea
                                        ...
                                        rows={3}
                                        maxLength={500}
                                        className="w-full px-3 py-2 border border-stone-200 rounded-lg text-sm resize-none focus:border-blue-600 bg-white outline-none"
                                      />
                                    </div>
                                    <div className="flex gap-2">
                                      ...
                                      <button
                                        ...
                                        className="px-4 py-1.5 text-xs font-bold text-white bg-blue-900 rounded-lg hover:bg-blue-950 disabled:opacity-40 transition-colors"
                                      >
                                  ...
                                  <button
                                    ...
                                    className="mt-1.5 flex items-center gap-1.5 text-xs font-semibold text-blue-900 hover:text-blue-950 transition-colors"
                                  >
                                ...
                                <div className="flex flex-col items-center flex-1">
                                  ...
                                  <span
                                    className={`text-xs mt-1 capitalize font-medium ${
                                      isCurrent ? 'text-blue-900' : isActive ? 'text-stone-600' : 'text-stone-400'
                                    }`}
                                  >
                                ...
                                {idx < 3 && (
                                  <div
                                    className={`h-0.5 flex-1 mb-5 transition-colors ${
                                      idx < currentIdx ? 'bg-blue-800' : 'bg-stone-200'
                                    }`}
                                  />
                                )}
                              </div>
